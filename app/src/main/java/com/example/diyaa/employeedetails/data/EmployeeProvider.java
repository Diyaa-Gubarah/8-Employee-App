...
public class EmployeeProvider extends ContentProvider {
    ...
    static {
        ...
        sUriMatcher.addURI(EmployeeContract.CONTENT_AUTHORITY, EmployeeContract.PATH_EMPLOYEES_TABLE, EMPLOYEES);

        sUriMatcher.addURI(EmployeeContract.CONTENT_AUTHORITY, EmployeeContract.PATH_EMPLOYEES_TABLE + "/#", EMPLOYEE_ID);
    }
    ...
    private Uri insertEmployee(Uri uri, ContentValues values) {
        ...
        String name = values.getAsString(EmployeeEntry.COLUMN_EMPLOYEE_NAME);
        if (name == null) {
            throw new IllegalArgumentException("employee requires a name");
        }

        // Check that the number is valid
        Integer number = values.getAsInteger(EmployeeEntry.COLUMN_EMPLOYEE_NUMBER);
        if (number == null) {
            throw new IllegalArgumentException("employee requires valid number");
        }
    ...
    private int updateEmployee(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        if (values.containsKey(EmployeeEntry.COLUMN_EMPLOYEE_NAME)) {
            String name = values.getAsString(EmployeeEntry.COLUMN_EMPLOYEE_NAME);
            if (name == null) {
        ...
        // check that the address value is not null.
        if (values.containsKey(EmployeeEntry.COLUMN_EMPLOYEE_ADDRESS)) {
            String address = values.getAsString(EmployeeEntry.COLUMN_EMPLOYEE_ADDRESS);
            if (address == null) {
                throw new IllegalArgumentException("Pet requires a name");
            }
        }
        ...
        if (values.containsKey(EmployeeEntry.COLUMN_EMPLOYEE_NUMBER)) {
            Integer number = values.getAsInteger(EmployeeEntry.COLUMN_EMPLOYEE_NUMBER);
            if (number == null) {
